...
$(document).ready(() => {
  ...
  function loadAssignedChoreIcons(kidId) {
    ...
    $.get(`api/assignedchores/${kidId}`, (assignedChores) => {
  ...
  //
  // ***************************
  function handleRequestApproval(kidId) {
  ...
  function handleParentApproval(kidId) {
    ...
    choresDone.forEach((choreDone) => {
      ...
      $.ajax({
        ...
        .then(() => {
          $.ajax({
            method: 'PUT',
            url: `/api/kids/1/add/${choreDone.points}`,
          })
            .then(() => {
              $.ajax({
                method: 'POST',
